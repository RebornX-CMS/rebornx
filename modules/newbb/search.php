...
if ( !isset($_POST['submit']) ) {
	...
	if ( isset($_POST['term']) && trim($_POST['term']) != "" ) {
		$terms = explode(' ', $myts->oopsAddSlashes($_POST['term']));		// Get all the words into an array
		if ( strlen($terms[0]) < 4 ) {

		}
		$addquery .= "(pt.post_text LIKE '%$terms[0]%'";	// Changed from %$terms[0]% to %$terms[0]% 
		$subquery .= "(t.topic_title LIKE '%$terms[0]%'";	// Changed from %$terms[0]% to %$terms[0]% 
